...
for i in range(len(water_districts_overall)):
    for k in range(len(water_districts_overall[i])):
        all_district_names.append("output/water_district_boundaries/" + water_district_names[i] + "/" +
    ...
    if water_agency_files_exist == True:
        ...
        for k in range(len(water_districts_overall[i])):
            ...
            in_layer_or_view = arcpy.MakeFeatureLayer_management(
                in_features="data/water_district_shape_files/WaterDistricts20180108.shp",
                out_layer=str(water_districts_overall[i][k]).replace("[", "").replace("]", "").replace("'", "").replace(" ", "_").replace("-","")
                )
            ...
            arcpy.SelectLayerByAttribute_management(
                in_layer_or_view=str(water_districts_overall[i][k]).replace("[", "").replace("]", "").replace("'", "").replace("-","").replace(" ", "_"),
                selection_type='NEW_SELECTION',
                where_clause=where_clause
                )

            #write to a new featureclass, outputs to a shapefile called test with the clipped data
            file_output_name = "output/water_district_boundaries/" \
            ...
            arcpy.CopyFeatures_management(
                in_features=str(water_districts_overall[i][k]).replace("[", "").replace("]", "").replace("'", "").replace("-","").replace(" ", "_"),
                out_feature_class=file_output_name
                )
